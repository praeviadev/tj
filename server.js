...
const { getPool, migrate, migrateTJ } = require('./db');
...
const TJ_SESSION_TTL = 7 * 24 * 60 * 60 * 1000; // 7 days ms
...
// ── Serve tj.html — BEFORE catch-all, no Covenant auth required ──────────────
app.get('/tj', (req, res) => {
  res.sendFile(path.join(__dirname, 'public', 'tj.html'));
});
...
async function tjLoadState() {
  ...
  const defaults = {
    ...
    rewardHistory: [],
    penaltyHistory: [],
    settings: { dailyGoal: 500, xpPerMin: 10, todoistProjectId: '' },
    xpLog: {}
  };
  await dbPool.query(
...
// POST /api/tj/auth  — password login, returns session token
app.post('/api/tj/auth', async (req, res) => {
  try {
    ...
    // Don't issue session yet — frontend will call /api/tj/pin next
...
// POST /api/tj/pin  — PIN login, issues/refreshes session token
app.post('/api/tj/pin', async (req, res) => {
  try {
    ...
    const token = tjToken();
    const now = Date.now();
    // Clean up old sessions for this role to avoid accumulation
...
// POST /api/tj/task/complete  — mark a task done
...
// POST /api/tj/reward/claim  — spend XP on a reward
app.post('/api/tj/reward/claim', tjRequireSession, async (req, res) => {
  try {
    ...
    if (process.env.TELEGRAM_N8N_WEBHOOK) {
      notifyTelegram(`🎮 TJ claimed reward: ${rewardName} (${xpCost} XP spent)\nBank: ${state.xp.total} XP`);
    }
    res.json({ ok: true, state });
  } catch(e) {
...
// POST /api/tj/settings  — parent-only settings update
...
// POST /api/tj/penalty  — parent issues a penalty
...
// POST /api/tj/log  — frontend error/event logging
app.post('/api/tj/log', async (req, res) => {
  try {
    const { event, payload } = req.body || {};
    await tjLogEvent(event || 'client_log', payload || {});
    // Forward errors to n8n logging webhook
    if (event?.includes('error') && process.env.TJ_LOG_WEBHOOK) {
      ...
      }).catch(e => console.warn('[tj/log forward]', e.message));
    }
    res.json({ ok: true });
  } catch(e) {
